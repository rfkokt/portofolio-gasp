...
export default async function PostsListPage({
  ...
  searchParams: Promise<{ filter?: string; page?: string; search?: string; author?: string }>;
}) {
  ...
  const result = await getPostsForCMS(filter, page, search, author);
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-8">
        <SearchInput />
        <AuthorFilter currentUser={currentUser} />
        <GenerateBlogButton />
        <Link
          href="/cms/posts/new"
          className="h-[40px] px-6 py-2 bg-foreground text-background font-bold text-sm uppercase tracking-wider hover:bg-foreground/90 transition-colors inline-flex items-center gap-2 rounded-md"
        >
